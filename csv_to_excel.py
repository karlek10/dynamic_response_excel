...
def csv_to_excel(file_name):
    ...
    emv = df[" mV"].astype(str)
    emv = emv.str.replace(",", ".")
    emv = emv.astype(float)
    return emv
...
def calculate_(name, dataframe):
    conc = float(name[-7])*10**(-1*float(name[-5]))
...
if __name__ == "__main__":
    ...
    for k, v in dict_files.items():
        ...
        k_sheet = k_fp + "_" + k_sp[-7:] # sheet names for the excel file
    ...
    for sheet in dict_files.keys(): # adding lines to summary chart
        ...
        k_sheet = sheet_fp + "_" + sheet_sp[-7:] # sheet names for the excel file
